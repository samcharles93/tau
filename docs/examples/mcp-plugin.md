...
MCP servers are subprocesses that speak JSON-RPC over stdio (or SSE/HTTP).
Tau's tool-calling model is Go-native — tools are Go functions returning a
result string. Rather than teach the coordinator a second tool protocol, the
MCP plugin does the translation once: it launches configured MCP servers as
child processes, lists their tools, and re-advertises each one as a
`pluginapi.ToolDefinition`. From the agent's point of view, an MCP tool looks
identical to a built-in tool or any other plugin tool.
...
Each entry's `command`/`args` is exactly what you'd run by hand to start that
MCP server — the plugin execs it and speaks MCP over its stdin/stdout.
