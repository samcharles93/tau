...
The agent coordinator (`internal/agent/coordinator.go`) is the central runtime that implements the `ChatRuntime`
interface. It owns the turn loop, tool execution, session state, and plugin lifecycle dispatch. Every client (TUI, Web
...
The coordinator receives `ChatCommand` values on an internal channel and publishes `ChatEvent` values on the event bus.
It does not know or care whether commands come from the TUI, the Web UI, or a headless stdin client.
...
The coordinator runs a `loop()` goroutine that selects on the command channel and schedule ticker. Each turn is
processed by `runTurn()`:

1. **Build request** - Build the system prompt (from templates + plugins + skills), collect tool schemas, assemble
   message history.
2. **Stream** - Call `Streamer.StreamChatCompletionFull()` with the assembled request.
3. **Receive deltas** - Process `OnDelta` (text), `OnReasoningDelta` (reasoning), `OnToolCallDelta` (tool calls)
   callbacks, emitting corresponding `ChatEvent` values.
...
- **`emit(event)`** - Non-blocking publish; if the bus channel is full, the event is dropped (used for streaming
  deltas).
- **`emitMustDeliver(event)`** - Bounded-blocking publish (500ms timeout); used for terminal events that must not be
  dropped (session snapshots, completion, errors).
...
The streamer is implemented by `internal/app/streamer.go` as a dynamic adapter that picks the correct ai-sdk provider
per turn based on the session's configured provider and model. It supports:
...
- **Base template**: `internal/agent/templates/agent.md.tpl` - the main system prompt, a Go text/template.
...
Beyond the base system prompt, tau ships a set of built-in agent commands (`/plan`, `/research`, `/rubber-duck`,
`/init`, `/compact`, `/summarise`) defined declaratively in `internal/agent/spec/templates/*.agent.md`. Each file is
Markdown with YAML frontmatter (`name`, `description`, `tools`, `argument-hint`, `user-invocable`, `metadata`) followed
by a Go text/template prompt body - the same frontmatter+body shape as `SKILL.md`, and loosely modeled on GitHub
Copilot's custom agent spec. `internal/agent/spec` (package `spec`) parses these; it has no dependency on the
coordinator or command registry so both can depend on it.

Invoking one (e.g. `/plan add dark mode`) replaces the session's system prompt with the rendered template, restricts the
tool registry to `tools:` (if set, via `Coordinator.SetAllowedTools`), and submits any trailing text as the next turn's
prompt. `model:` and `disable-model-invocation:` are parsed but not yet enforced

- reserved for a future per-agent model override and model-driven agent selection, respectively.
...
Plugins can respond with modifications: inject messages, remove messages, append system prompt text, block tools, modify
tool arguments/results, add HTTP headers, or change the model ID.
