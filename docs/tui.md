...
Tau's terminal UI is a reactive, inline-rendering chat interface built on the `pkg/taui` framework. It renders directly
into the terminal scrollback (no alternate screen) and communicates with the coordinator through the event bus.
...
Plugin slash commands are dynamically added to the table when plugins are loaded. They appear as
`/plugin:<name>:<command>`.
