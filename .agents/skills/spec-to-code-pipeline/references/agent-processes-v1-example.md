...
An illustration of all 6 phases applied to a real project. Source: a gap-review document covering 7 spec files across
storage, wire protocol, lifecycle, and UI domains. 18 gaps → 18 tickets → completed in 15 commits.

## Phase 1: Spec Review

Read 11 documents: overview, spec format, lifecycle, wire protocol, storage, UI, implementation plan, plus 3
design-decision records (ADRs) and the gap-review document itself.
...
18 tickets in a single project, conventional-commit titled, each with Gap/Scope/Acceptance sections. Examples of scope
prefixes: `fix(storage):`, `feat(wire):`, `docs(spec):`, `test(integration):`.
...
The resume-authorization ticket (T-process-1) had 5 upstream dependencies across design, storage, and wire layers. All
had to complete before its contract could be specified.
