...
- `internal/chat/types.go` — All public types:
  ...
  - **Core types**: `ChatMessage`, `ToolResultMetadata`, `ChatRole`, `ChatSessionState`, `ChatSessionConfig`, `ChatSessionPatch`, `ChatSessionStatus`, `ChatParameters`, `ChatModelRef`, `ChatToolCall`, `ChatToolDef`, `ChatUsage`, `CommandRef`, `ExtensionCommand`, `ExtensionReloader`, `SessionSummary`, `ChatCompletionRequest`, `ChatCompletionResponse`, `ChatCompletionChunk`, `ChatToolCallDelta`, etc. Tool-result messages persist authoritative status, error kind, duration, truncation, byte count, and execution timestamps in `ToolResultMetadata`; analytics must prefer it over parsing result prose.
